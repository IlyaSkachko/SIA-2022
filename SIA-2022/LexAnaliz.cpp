...
namespace Lexer
{
		...
		for (int i = 0; i < ittable.size; i++)
		{
			if (ittable.table[i].idtype == IT::IDTYPE::L && ittable.table[i].iddatatype == type)
			{
				...
				case IT::IDDATATYPE::BOOL:
					char bbuf[BOOL_MAXSIZE];
					for (unsigned j = 0; j < strlen(value); j++)
					{
						bbuf[j] = value[j];
					}
					if (strcmp(ittable.table[i].value.vbool.str, bbuf) == 0)
						return i;
					break;
				}
			}
	...
	{
		if (isdigit(*id) || *id == IN_CODE_QUOTE || *id == LEX_MINUS || *id == IN_CODE_NOT_DOUBLE_QUOTE || (id[0] == 't' && id[1] == 'r' && id[2] == 'u' && id[3] == 'e') || (id[0] == 'f' && id[1] == 'a' && id[2] == 'l' && id[3] == 's' && id[4] == 'e'))
			return true;
		return false;
	}
		...
		bool& lex_ok)							// флаг ошибки(по ссылке)
	{
		...
		if (lex == LEX_LITERAL) // литерал
		{
			...
			if (int_ok && itentry->iddatatype == IT::IDDATATYPE::NUM)
			{
				char p[11];
		...
		else // идентификатор
		{
			...
			case IT::IDDATATYPE::NUM:
				...
				break;
			case IT::IDDATATYPE::SYM:
				itentry->value.symbol = SYM_DEFAULT;
				break;			
			case IT::IDDATATYPE::BOOL:
				itentry->value.vbool.len = BOOL_DEFAULT;
				break;
			}
			...
			for (int j = 0; j < N_GRAPHS; j++)
			{
				...
				if (FST::execute(fst))
				{
					...
					case LEX_LITERAL:
					{
						...
						if (*nextword == LEX_LEFTHESIS || IT::isId(tables.idtable, curword) != NULLIDX_TI)
						{
							isFunc = true;
							if (getStandFunction(curword) == IT::STDFNC::F_NOT_STD)
								strncat(id, "_", MAXSIZE_ID);
						}
