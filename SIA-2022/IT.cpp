...
namespace IT
{
	...
	bool SetValue(IT::Entry* entry, char* value) // óñòàíîâêà çíà÷åíèÿ ïåðåìåííîé
	{
		...
		{
			for (int i = 1; i < strlen(value) - 1; i++)	// áåç êàâû÷åê
		...
		else if (entry->iddatatype == BOOL)
		{
			...
			{
				entry->value.vbool.str[i] = value[i];
			}
			entry->value.vbool.len = strlen(value);
		}
		return rc;
	}
		...
		*stream << "|  N  |ÑÒÐÎÊÀ Â ÒË| ÒÈÏ ÈÄÅÍÒÈÔÈÊÀÒÎÐÀ |        ÈÌß        | ÇÍÀ×ÅÍÈÅ (ÏÀÐÀÌÅÒÐÛ)" << std::endl;
		for (int i = 0; i < idtable.size; i++)
		{
			...
			if (e->idtype == IT::IDTYPE::L || e->idtype == IT::IDTYPE::V && e->iddatatype != IT::IDDATATYPE::UNDEF)
			{
				...
				else if (e->iddatatype == IT::IDDATATYPE::SYM)
					*stream << e->value.symbol;
				else if (e->iddatatype == IT::IDDATATYPE::BOOL)
				{
					*stream << "[" << (int)e->value.vbool.len << "]";
					for (int i = 0; i < e->value.vbool.len; i++)
					{
						*stream << e->value.vbool.str[i];
					}
				}
			}
